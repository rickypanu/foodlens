...
import { useAuth } from "../src/context/AuthContext"; // <--- Import this
...
const handleStartTrial = async () => {
    ...
    try {
      // 1. Check if we have the user data passed from Signup
      if (params.token && params.user) {
        const userObj = JSON.parse(params.user as string);
        
        // 2. ACTUALLY LOG THEM IN NOW
        await login(params.token as string, userObj);
        
        // 3. Navigate to Home
        router.replace('/(tabs)');
      } else {
        // Fallback (e.g., if testing screen without signup)
        router.replace('/(tabs)');
      }
    } catch (error) {
  ...
  useEffect(() => {
    handleStartTrial(); // <--- 3. Automatically triggers login & redirect
  }, []);
...
<View style={{ flex: 1, justifyContent: "center", alignItems: "center", backgroundColor: "#fff" }}>
      ...
      <Text style={{ marginTop: 20, color: "#666" }}>Finalizing account...</Text>
    </View>
  );
}
...
//             Eat smarter for less than a snack
//           </Text>
...
//         <TouchableOpacity onPress={handleStartTrial} style={styles.footerBtn}>
//           <LinearGradient
